$(function(){
	...
	$.ajax({
		...
		success:function(str){
			...
			if(obj.payTime==undefined){
				obj.payTime='';
				//alert(obj.payTime)
				$('#payTime').val(obj.payTime);
			}else{
				$('#payTime').val(Uiho.tool.DetailTimesTamp(obj.payTime));
			}
			$('#startTime').val(bTime);
			$('#endTime').val(Etime);
			$('#appoTime').val(Uiho.tool.DetailTimesTamp(obj.createTime));
			$('#courseTime').val(Uiho.tool.DetailTimesTamp(obj.courseDate).substring(0,11));
			
			
			$('#payway').val(payChannels);
